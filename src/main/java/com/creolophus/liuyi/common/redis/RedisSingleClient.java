...
public class RedisSingleClient implements RedisClient{
    ...
    public boolean lockWaiting(String key,long seconds,long millions){
        ...
        while(!_lock(key,seconds)){
            long begin = beginTable.computeIfAbsent(threadKey, s -> System.currentTimeMillis());
            if((begin+millions)<=System.currentTimeMillis()){
                beginTable.remove(threadKey);
                return false;
            }
        }
        beginTable.remove(threadKey);
    ...
    private boolean _lock(String key,long seconds){
        return StringUtils.isNotBlank(lock(key,seconds));
    }
