...
public class LoggerAutoConfig {
    ...
    @ConditionalOnMissingBean
    public EntryTraceAspect traceAspect(Tracer tracer, SleuthSchedulingProperties sleuthSchedulingProperties) {
        logger.info("start... trace logger");
        return new EntryTraceAspect(tracer, Pattern.compile(sleuthSchedulingProperties.getSkipPattern()));
    }
